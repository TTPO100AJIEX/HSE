...
.input_from_file:
	...
	test rax, rax
	je .error_input_print # if (!freopen(argv[output_flag_argv_index++], "r", stdin))
...
.output_to_file:
	...
	test rax, rax
	je .error_output_print # if (!freopen(argv[output_flag_argv_index + 1], "w", stdout))
...
.error_cli_print:
	lea	rdi, .error_cli[rip] # rdi <-- address of "Incorrect command line arguments provided!" - first function argument
	mov	eax, 0 # we do not expect printf() to use coprocessors (x87, SSE, etc.)
	call printf@PLT # printf("Input too large!");
	jmp .main_return
.error_input_print:
	lea	rdi, .error_input[rip] # rdi <-- address of "Failed to open an input file!" - first function argument
	mov	eax, 0 # we do not expect printf() to use coprocessors (x87, SSE, etc.)
	call printf@PLT # printf("Input too large!");
	jmp .main_return
.error_output_print:
	lea	rdi, .error_output[rip] # rdi <-- address of "Failed to open an output file!" - first function argument
	mov	eax, 0 # we do not expect printf() to use coprocessors (x87, SSE, etc.)
	call printf@PLT # printf("Input too large!");
	jmp .main_return
