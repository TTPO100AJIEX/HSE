...
for (let executable of to_test)
{
    ...
    for (let tg = 0; tg < test_groups.length; tg++)
    {
        ...
        {
            execFileSync(`./${executable}`, [ "0", "1", `testing/tests/group${tg + 1}/${i}.in`, "1", "int/tmp.out" ]);
            let output = fs.readFileSync(`testing/tests/group${tg + 1}/${i}.out`).toString("utf-8");
            let answer = fs.readFileSync(`int/tmp.out`).toString("utf-8"); answer = answer.substring(0, answer.indexOf("\n"));
