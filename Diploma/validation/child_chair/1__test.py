...
train_features = numpy.hstack(
    [numpy.load(f"1/{media_code}/train_features.npy") for media_code in ["front", "label", "side"]]
)

test_features = numpy.hstack(
    [numpy.load(f"1/{media_code}/test_features.npy") for media_code in ["front", "label", "side"]]
)
