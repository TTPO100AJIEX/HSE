...
train_labels = numpy.array(
    [labels_map[item[0][len("thermobag/train/train/") : -len("_thermobag.jpg")]] for item in tqdm.tqdm(train.samples)]
)

test_images = numpy.array([numpy.array(item[0]) / 255 for item in tqdm.tqdm(test)])
test_labels = numpy.array(
    [labels_map[item[0][len("thermobag/test/test/") : -len("_thermobag.jpg")]] for item in tqdm.tqdm(test.samples)]
)
