...
def process(name, train, test, with_diagrams: bool = True, with_labels: bool = True, should_clean: bool = True):
    folder = f"{name}/results"
    if args.clean and should_clean:
        shutil.rmtree(folder, ignore_errors = True)
    ...
    return process_impl(
        name, train_images, train_labels, test_images, test_labels, with_diagrams, with_labels, should_clean
    )

def classification(name, train, test):
    start = time.time()
    results = cvtda.classification.classify(
        *process(name, train, test, with_diagrams = not args.quick), dump_name = f"{name}/results/classification",
        only_get_from_dump = False, **classification_params
    )
...
def face_recognition(name, train, test):
    ...
    fig = cvtda.face_recognition.learn(
        *process(name, train, test), dump_name = folder, **face_recognition_params
    )
    fig.suptitle("AT&T Database of Faces", fontsize = 12)
    fig.tight_layout()
...
def compression(name, train, test):
    ...
    results = cvtda.autoencoder.try_autoencoders(
        *process(name, train, test, with_labels = False, should_clean = False),
        dump_name = f"{name}/results/compression", only_get_from_dump = False, **autoencoders_params
    )
...
def segmentation(name):
    ...
    train_images, train_features, train_masks, _, test_images, test_features, test_masks, _ = process_impl(
        name, *load_dataset(f"{name}/train"), *load_dataset(f"{name}/val"), with_diagrams = False
    )
    result = cvtda.segmentation.segment(
        train_images, train_features, train_masks, test_images, test_features, test_masks,
        dump_name = f"{name}/results/segmentation", only_get_from_dump = False, 
        remove_cross_maps = True, **segmentation_params
    )
...
with cvtda.logging.DevNullLogger():
    ...
    classification(
        ...
        torchvision.datasets.Imagenette(
            'imagenette', split = 'val', size = '160px', transform = transform, download = False
        )
    )
