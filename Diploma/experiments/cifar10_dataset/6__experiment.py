...
transform = torchvision.transforms.v2.Compose(
    [torchvision.transforms.v2.ToImage(), torchvision.transforms.v2.ToDtype(torch.float32, scale=True)]
)
...
for i, filtration in enumerate(greyscale_to_filtrations.filtrations_):
    ...
    make_features(
        f"{type(filtration).__name__}{i}", binarizer=gtda.images.Binarizer(threshold=0.4), filtration=filtration
    )
