...
def make_features(
    channel: int, split_idx: typing.Optional[int], inverter, binarizer, filtration, n_jobs: int = 1
...
def process(
    inverted: bool, channel: int, split_idx: typing.Optional[int], binarizer_threshold: float
) -> typing.Tuple[numpy.ndarray, numpy.ndarray]:
    ...
    greyscale_to_filtrations = cvtda.topology.GreyscaleToFiltrations(
        n_jobs=1, radial_filtration_centers=list(itertools.product(centers, centers))
    )
...
for binarizer_threshold_mul in range(1, 9):
    ...
    for channel in ["red", "green", "blue", "gray", "saturation", "value"]:
        print(
            f">>> Calculating channel {channel} for an inverted image with binarizer_threshold = {binarizer_threshold}"
        )
