...
from cvtda.utils import set_random_seed, FlattenBatch
...
class GreyscaleToFeatures(sklearn.base.TransformerMixin):
    def __init__(
        self,
        n_jobs: int = -1,
        verbose: bool = True,
        ...
        self.greyscale_to_filtrations_ = GreyscaleToFiltrations(
            binarizer_threshold = binarizer_threshold,
            radial_filtration_centers = list(itertools.product(radial_filtration_centers_x, radial_filtration_centers_y))
        )
        self.filtrations_flatten_ = FlattenBatch()
        self.filtrations_to_diagrams_ = FiltrationsToDiagrams()
        self.filtration_features_ = DiagramsToFeatures(n_bins = 32)

        self.with_pointclouds_ = with_pointclouds
        if with_pointclouds:
