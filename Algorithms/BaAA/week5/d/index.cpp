...
int main()
{
    ...
    std::sort(intervals.begin(), intervals.end(), static_cast<bool(*)(const Interval&, const Interval&)>(operator<));
    ...
    {
        unsigned int lastTakenIndex = std::upper_bound(intervals.begin(), cur, std::get<0>(*cur), static_cast<bool(*)(const double, const Interval&)>(operator<)) - intervals.begin();
