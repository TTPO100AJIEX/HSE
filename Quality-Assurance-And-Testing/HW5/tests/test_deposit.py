...
@pytest.mark.parametrize([ 'server_code', 'server_response', 'expected_code', 'expected_response' ], [
    ...
    [ ServerResponse.NO_MONEY,                   AMOUNT - 5, AccountManagerResponse.INCORRECT_RESPONSE, ServerResponse(ServerResponse.NO_MONEY, AMOUNT - 5) ],
...
def test_different_server_responses(
    ...
    deposit_stats = mock_server("deposit", ( SESSION_ID, AMOUNT ), server_code, server_response)

    response = account_manager_logged.deposit(LOGIN, SESSION_ID, AMOUNT)
    ...
    assert deposit_stats['ok'] == True
    assert deposit_stats['calls'] == 1
...
def test_no_logged(account_manager):
    response = account_manager.deposit(LOGIN, SESSION_ID, AMOUNT)
