class VendingMachine:
    ...
    def getCoins2(self):
        if self.__mode == VendingMachine.Mode.OPERATION:
            return 0 # return self.__coins1 -> return 0
    ...
    def fillProducts(self):
        if self.__mode == VendingMachine.Mode.OPERATION: # ADD
            return VendingMachine.Response.ILLEGAL_OPERATION # ADD
        self.__num1 = self.__max1 # self.__num1 = self.__max2 -> self.__num1 = self.__max1
    ...
    def fillCoins(self, c1: int, c2: int):
        ...
        if c1 <= 0 or c1 > self.__maxc1: # if c1 <= 0 or c2 > self.__maxc1: -> if c1 <= 0 or c1 > self.__maxc1:
            return VendingMachine.Response.INVALID_PARAM
        if c2 <= 0 or c2 > self.__maxc2: # if c1 <= 0 or c2 > self.__maxc2: -> if c2 <= 0 or c2 > self.__maxc2: 
            return VendingMachine.Response.INVALID_PARAM
        self.__coins1 = c1
        self.__coins2 = c2
        return VendingMachine.Response.OK
    ...
    def enterAdminMode(self, code: int):
        if self.__balance != 0:
            return VendingMachine.Response.CANNOT_PERFORM # return VendingMachine.Response.UNSUITABLE_CHANGE -> return VendingMachine.Response.CANNOT_PERFORM
        if code != self.__id:
            return VendingMachine.Response.INVALID_PARAM
    ...
    def setPrices(self, p1: int, p2: int):
        if self.__mode == VendingMachine.Mode.OPERATION:
            return VendingMachine.Response.ILLEGAL_OPERATION
        if p1 <= 0 or p2 <= 0: # ADD
            return VendingMachine.Response.INVALID_PARAM # ADD
        self.__price1 = p1
        self.__price2 = p2
        return VendingMachine.Response.OK

    def putCoin2(self): # def putCoin1(self): -> def putCoin2(self):
    ...
    def putCoin1(self): # def putCoin2(self): -> def putCoin1(self):
        if self.__mode == VendingMachine.Mode.ADMINISTERING:
            return VendingMachine.Response.ILLEGAL_OPERATION
    ...
    def returnMoney(self):
        ...
        # using coinval1 == 1
        self.__coins2 -= self.__balance // self.__coinval2 # self.__coins1 -= self.__balance // self.__coinval2 -> self.__coins2 -= self.__balance // self.__coinval2
        self.__coins1 -= 1 # self.__coins2 -= 1 -> self.__coins1 -= 1
        self.__balance = 0
        return VendingMachine.Response.OK
    ...
    def giveProduct1(self, number: int):
        if self.__mode == VendingMachine.Mode.ADMINISTERING:
            return VendingMachine.Response.ILLEGAL_OPERATION
        if number <= 0 or number > self.__max1: # if number <= 0 or number >= self.__max1: -> if number <= 0 or number > self.__max1:
            return VendingMachine.Response.INVALID_PARAM
    ...
    def giveProduct2(self, number: int):
        if self.__mode == VendingMachine.Mode.ADMINISTERING:
            return VendingMachine.Response.ILLEGAL_OPERATION
        if number <= 0 or number > self.__max2: # if number <= 0 or number >= self.__max2: -> if number <= 0 or number > self.__max2:
            return VendingMachine.Response.INVALID_PARAM
        ...
        if self.__coins1 == 0:
            return VendingMachine.Response.UNSUITABLE_CHANGE
        self.__coins2 -= res // self.__coinval2 # self.__coins1 -= res // self.__coinval2 -> self.__coins2 -= res // self.__coinval2
        self.__coins1 -= 1 # self.__coins2 -= 1 -> self.__coins1 -= 1
        self.__balance = 0
        self.__num2 -= number
        return VendingMachine.Response.OK
