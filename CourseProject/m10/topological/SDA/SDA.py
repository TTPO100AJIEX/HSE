...
from .clustquality.apply_cluster_method import apply_cluster_method
from .clustquality.cluster_metrics_noground import cluster_metrics_noground
from .clustquality.calc_stage_metr_noground import calc_stage_metr_noground

from .stageprocess.form_stages import form_stages
from .stageprocess.form_stage_bands import form_stage_bands
from .stageprocess.calc_stage_distances import calc_stage_distances
from .stageprocess.merge_stages_1st_step import merge_stages_1st_step
from .stageprocess.merge_stages_2nd_step import merge_stages_2nd_step
...
class SDA:
    ...
    def stage1(self, features: numpy.ndarray) -> pandas.DataFrame:
        ...
        for (n_clusters, k_neighbours) in tqdm.contrib.itertools.product(self.n_clusters, self.k_neighbours):
            ...
            _, labels, metrics = apply_cluster_method(features, AgglomerativeClustering, **kwargs)
    ...
    def stage2(self, features: numpy.ndarray, df_st_edges: pandas.DataFrame) -> pandas.DataFrame:
        ...
        for (st_len, k_nb_max, n_cl, n_st_edge_clusters) in tqdm.contrib.itertools.product(self.len_st_thr, self.k_neighb_max_thr, self.n_cl_max_thr, self.n_st_edge_clusters):
            ...
            _, labels, _ = apply_cluster_method(st_edges_all, KMeans, **kwargs)
